...
MyVehicle::MyVehicle():Vehicle(){
	...
	CYL::Cylinder * front_tyre = new CYL::Cylinder(x + 1, y - 0.5, z, 0.2, 0.5, 22, 0.05, 1, 0.05, rotation);
	CYL::Cylinder * front_rim = new CYL::Cylinder(x + 1, y - 0.2, z, 0, 0.2, 2.1, 0.75, 0.75, 0.75, rotation);
	CYL::Cylinder * back_tyre = new CYL::Cylinder(x - 1, y - 0.5, z, 0.2, 0.5, 22, 0.05, 1, 0.05, rotation);
...
void MyVehicle::draw(){
	glPushMatrix();
	shapes[2]->setRotation(steering);
	shapes[3]->setRotation(steering);
	this->positionInGL();
